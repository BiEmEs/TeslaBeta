...
public class Pinnable {
	...
	public Pinnable(String title, String tag, float font_, int x, int y) {
		this.title = title;
		this.tag   = tag;
		this.font  = new ClientDraw(font_);
	...
	public void crush(int mx, int my) {
		...
		int screen_x = (mc.getWindow().getWidth() / 2);
		int screen_y = (mc.getWindow().getHeight() / 2);
	...
	public void render(int mx, int my, int tick) {
		...
		if (this.x + this.width <= (mc.getWindow().getWidth() / 2) / 2) {
			set_dock(true);
		} else if (this.x + this.width >= (mc.getWindow().getWidth() / 2) / 2) {
			set_dock(false);
		}
		...
		if (is_active()) {
			...
			if (motion(mx, my)) {
				ClientDraw.draw_rect(this.x - 1, this.y - 1, this.width + 1, this.height + 1, 0, 0, 0, 90, 2, "right-left-down-up");
			}
		}
	}

	protected void create_line(String string, int pos_x, int pos_y) {
		ClientDraw.draw_string(string, this.x + pos_x, this.y + pos_y, 255, 255, 255, 255);
	}

	protected void create_line(String string, int pos_x, int pos_y, int r, int g, int b, int a) {
		ClientDraw.draw_string(string, this.x + pos_x, this.y + pos_y, r, g, b, a);
	}

	protected void create_rect(int pos_x, int pos_y, int width, int height, int r, int g, int b, int a) {
		ClientDraw.draw_rect(this.x + pos_x, this.y + pos_y, this.x + width, this.y + height, r, g, b, a);
	}
